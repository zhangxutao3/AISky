...
- 可持久化的自动同步开关；开启时立即并行检查两个模型，之后每 3 小时检查最新固定起报
- Energy 与 SDS 使用独立数据进程下载，右上角轮播各模型进度与实时下载速度
...
- 每次冷启动先显示品牌封面，数据与地图就绪后再进入；无完整数据时显示 120 时次初始化进度
- 设置中可迁移完整数据目录，迁移完成后自动重启并清理受控的旧目录
- 下载断线自动重试、损坏 NetCDF 隔离、损坏 SQLite 自动备份重建
- 内置数据运行时，发布包不再依赖电脑预装 Python

应用数据默认保存在程序目录下的 `AISkyData`，也可在“设置 → 数据存储”中选择
其他空文件夹。安装版的默认结构为：

```text
<AISky 安装目录>\AISkyData\data      原始 NetCDF
<AISky 安装目录>\AISkyData\cache     SQLite 索引和地图渲染缓存
<AISky 安装目录>\AISkyData\logs      运行日志
<AISky 安装目录>\AISkyData\config    用户设置
```
...
近期完整模型，并为缺失或陈旧模型同步未来 15 天的 120 个预报时次；两个模型由
独立工作进程并行执行。之后每 3 小时再次检查。任务运行时，右上角轮播各模型状态、
进度和下载速度。
...
[`docs/RELEASING.md`](docs/RELEASING.md)。开启更新检查后，AISky 每 6 小时在后台
静默检查一次；发现新版本时再弹出提示。

普通用户请阅读 [`docs/USER_GUIDE.md`](docs/USER_GUIDE.md)，测试结果见
[`docs/TEST_REPORT-v0.8.6.md`](docs/TEST_REPORT-v0.8.6.md)。
